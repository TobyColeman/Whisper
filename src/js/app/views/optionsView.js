...
define("optionsView", ['Utils', 'EventManager', 'StoreController'], function(
    Utils, EventManager, StoreController) {
    ...
    Utils.forEachChild(document.documentElement, function(el) {
        ...
        if (el.tagName == 'INPUT' || el.tagName == 'TEXTAREA') {
            ...
            if (!!el.childNodes.length) {
                el.childNodes[0].innerHTML = chrome.i18n.getMessage(
                    resourceName);
            } else {
    ...
    function bindEvent() {
        // form events
        document.forms["keyGenForm"].addEventListener('submit',
            handleKeyForm);
        document.forms["keyInsForm"].addEventListener('submit',
            handlePrivInsert);
        document.forms["friendInsForm"].addEventListener('submit',
            handlePubInsert);
        document.forms["delForm"].addEventListener('submit', doDelete);

        // click events
        document.getElementById('keyOpts').addEventListener('change',
            toggleKeyGenType);
        document.getElementById('friendFormToggle').addEventListener(
            'click', toggleFriendForm);
    ...
    function renderError(data) {
        ...
        errorBlock.children.namedItem('blockText').innerHTML =
            'Error: ' + data.error;
        document.getElementById('keyGenProgress').style.display =
            "none";
    ...
    function updateTableRows(keys, table) {
        ...
        keys.forEach(function(key, index) {
            ...
            deleteBtn.className =
                "ion-trash-b ion-medium ion-clickable";
            ...
            if (key.privKey != null) {
                ...
                deleteBtn.setAttribute('data-uid',
                    'whisper_key');
            } else {
    ...
    function promptDelete(e) {
        ...
        function updateModal() {
            ...
            modal.children.namedItem("delMsg").children.namedItem(
                "delName").innerHTML = name;
    ...
    function doDelete(e) {
        ...
        StoreController.delKey(keyId, function(success) {
            ...
            } else if (document.getElementById('friend_table').rows
                .length === 2) {
    ...
    function showKeyDetails(e) {
        ...
        function updateModal(key) {
            ...
            modal.children.namedItem('modalHeading').innerHTML =
                'Key For: ' + key.getName() + " - " + key.vanityID;

            if (key.privKey === null) {
                modal.children.namedItem('privKeyText').style.display =
                    "none";
                modal.children.namedItem('privateHeading').style.display =
                    "none";
            } else {
                modal.children.namedItem('privKeyText').style.display =
                    "block";
                modal.children.namedItem('privateHeading').style.display =
                    "block";
                modal.children.namedItem('privKeyText').innerHTML = key
                    .privKey.armor();
            }
